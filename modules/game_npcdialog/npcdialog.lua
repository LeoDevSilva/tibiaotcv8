...
function init()
	...
	buttonText = windowDialog:getChildById('sendText')
	
	local texteInput = windowDialog:getChildById('texteInput')
	
	buttonText.onClick = function()
        local text = texteInput:getText()
        if text ~= "" then
            g_game.talkChannel(11, 10, text)
			texteInput:setText("")
        end
    end
...
function createDialog(value)
	...
	local pathOn = "images/".. Npc:getName() ..".png"
	outfitBox:setImageSource(pathOn)
